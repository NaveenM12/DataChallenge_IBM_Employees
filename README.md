...
Beyond the results of the data, as an aside, there are major issues with the HR survey that can be corrected to prevent the need for a majority of the data cleaning. First, variables that are shared or arbitrary, such as employee count or employee number, should be removed from the dataset. Additionally, one glaring point was that the 'performance review' metric (as depicted below) becane arbirtrary because everyone was essentially given the same, postive response. In order to improve, people have to be honest with another, so the department should also look into making people feel more safe to speak out and criticize one another.
